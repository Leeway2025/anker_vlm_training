...
用法(两种鉴权,与标注器一致):
    ...
    python -m annotation.translate_chains --in pass1.jsonl \
        --out pass1_en.jsonl --api-key <KEY>
    #   (或 export GOOGLE_API_KEY=<KEY> 后省略 --api-key)
    # ② Vertex ADC 方式: 加 --vertex-project <P>
...
def main():
    ...
    client = (genai.Client(vertexai=True, project=args.vertex_project,
              ...
              else genai.Client(api_key=args.api_key))   # None → SDK 读环境变量
