...
- **⚠️ 朴素"错例×3"的整体下滑风险与防护**(必读):
  ...
  被挤压,总 ACC 可能净降;且"模型错"里混着"GT 错"(h/n 噪声 ~15%),
  放大即教错。三层防护(hard_mining 已内置):
  ① `--max-class-inflation 1.5`: 每类训练质量膨胀封顶(默认开);
  ② `--whitelist DATA/asset_D_whitelist.txt`: 只放大 Gemini 确认过
    GT 的可信错例,疑似噪声计入 wrong_untrusted 上报不放大;
  ③ 续训必须带早停 + 监控集(train.py 默认 load_best);
  推荐命令: `python -m training.hard_mining --preds preds_v0.jsonl
  --labels <labels> --whitelist DATA/asset_D_whitelist.txt --out sw.json`
- **验收标准**: 续训后监控集**总指标不降**且热区对(h/n、k/l、C/D)
  混淆减少;stats 里 wrong_untrusted 占错例比例过高(>30%)→ 先做
  数据审计再续训;class_inflation 表逐类过目。
- **实测**: (演练进行中,待补)
