...
## Step 9 1M 自然分布校准轮(第二轮,configs/phase7_1m_calib.yaml)

- **目的**: 把 balanced 100k 训出的 v2 校准到自然分布先验(测试集
  本就是自然分布,m 占 27.5% vs 训练 12.7%)。**是校准不是重训**——
  能力已注入,HM/5b/5d/资产标注全部不重跑。
...
- **注意事项**: ① LR×0.3 小步校准,大 LR 会冲掉 100k 轮成果;
  ...
  ③ v6e-8 上 1 epoch ≈ 1.5~2.5 天(bs4+accum8)。
