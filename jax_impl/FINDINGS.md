...
## v7(2026-07-21): prod 方案训不进分类字母 —— 有效步长过热(定案)

**症状**: 客户 98k 正式轮 SubKS 5.95%(低于无脑基线),val/seen 自考
同塌;预测塌缩到常数字母;desc 与格式完好。

**排查链**(全部实证,按序排除): 推理链路(remove_mm_logits 修复后
格式 100%)→ 字典口径(train/test/taxonomy/prompt 四处同源)→ 标签
内容与权重(dump 逐位正确 4/4/4/4)→ 帧-标签配对(成员名机制保证)
→ prefetch 组批(futures 顺序收取)→ 前向视觉通路(两视频标签位
logits maxdiff 17.3,不盲)→ 权重更新(B RMS 9e-3,在动)。

**判决实验**(64 视频,结构可分合成帧,300 步过拟合):
prod 默认(lr 1e-4, LoRA+16, scale 32/45)→ 常数字母 0%;
prod + A 初始化 std=1/r → 仍常数 0%;
uniform r16(无 scale)→ 100%;
**prod + lr 2e-5 + LoRA+1(冷)→ 100%**。

**结论**: rsLoRA scale × lr × LoRA+ 的有效步长约为 uniform 的
50-100×,长程"视频→字母"回路早期被打饱和,收敛到常数(边际众数);
desc 等局部文本模式鲁棒存活 —— 这也是 loss 曲线看似正常的原因。
prod 必须配冷 lr(2e-5, LoRA+1),train_sft 已加运行时警告;
A 初始化同步对齐 peft gaussian(std=1/r)。

**方法论沉淀**: ① 新训练管线上线前必须做"合成可分数据过拟合 100%"
门禁(本项目补做时间: 投产后 6 天);② 烟测输出为垃圾时必须跑 base
对照组;③ "loss 正常下降"不证明分类学会 —— desc/格式会撑住曲线;
④ torch 侧同款超参组合未经字母级验证,对拍时需同做过拟合门禁。

## v8(2026-07-21): S1→S9 全流程双轨门禁验证 —— 全绿

用户要求"其它阶段也完整验证"。双 SPOT v6e-4 并行,两套数据两种口径:

**A 轨(机制门禁,合成可分帧 64 条,考记忆)**: S1 proj 产出 ✅;
S2 接 S1 衔接(全零 LoRA 跳过)98.4% ✅;S3 挖掘→sw→加权续训 98.4% ✅;
S4 辅助头(aux 16 叶非零)100% ✅;S5 CoT(退火触发/think 零泄漏)
98.4% ✅;S6 KTO(prod 严格加载)✅;S7 SWA 98.4% ✅;S9 导出 714
tensors(vision 224)✅。

**B 轨(真实学习,UCF101 子集 405 条真人视频 10 类,按拍摄组切分,
考泛化)**: R2 主训(prod 自动冷配方,300 步)**测试集 99.0% /
训练集 100%**(随机基线 2%);R3 难例 99.0%;R4 辅助头 99.0%;
R5 CoT 99.0% + 格式 97/97;R6 KTO ✅;R7 SWA 95.9%;R9 导出 714 ✅。
数据资产: gs://leeway-main-ml-tmp/realdata_ucf10_20260721.tgz
(生产格式,随用随取;类别→字母程序化映射)。

**结论**: 全管线(含 v7 冷配方与全部修复)在真实视频上端到端
"能学-能评-能导出";各增强阶段均不损伤主指标。结合客户数据 6% 的
现象,问题定性进一步偏向客户侧标签质量/类别边界(待其冷配方重训
与标注一致性审计闭环)。
**沉淀**: 双轨门禁(机制轨+真数据轨)进入发版规程: 训练动力学类
改动须双轨全绿再交付;验证机引导清单补 python3-numpy;巡检 pgrep
一律 [x] 括号防自匹配。

## v9(2026-07-22): 客户真数据冷配方重训 —— 修复兑现,追平自训基线

客户 98k 人工标注数据,prod 冷配方(v7)重训后评测(11,022 条测试):
RoleType 80.84% / SubKS 69.24% / KeyScene 91.17%
(修复前: 34.30% / 5.95% / 71.87%;SubKS 提升 11.6×)。
客户口径: 与其此前自训 Gemma E2B 的结果相当 —— 对拍以"同精度 +
~1.9× 吞吐"形式闭环,JAX 主路线成立。
后续提升排序: ① lr 中间档标定(2e-5 或偏保守,单变量对照+过拟合
门禁)② S3 难例挖掘(现有真实错例,S3/KTO 开始有意义)③ 标注
一致性审计(决定天花板)④ 资产齐备后 S4/S5。

## v10(2026-07-22): v4(去重+增强+早停)全面新高,超越客户同型基线

v4 = labels_dedup(去除 15.1% 复制行)+ --augment(翻转/亮度/帧dropout)
+ 早停(patience 3, 预算 1500 步),从 proj_a 重头,prod 冷配方:
  RT 82.75%(+1.91) / SubKS 72.38%(+3.14) / KS 91.82%(+0.65)
坐标: 超客户 Gemma4n-E2B 基线(69.59)+2.79,亦超其 E4B(71.87);
距 100k 档最强(EunoVLM 73.83 / Qwen3.5 74.4)约 1.5 分。
增益与预测吻合(augment +1~3, 去重 0~+1)。v3 教训(纯加步数过拟合
-1.4)与 v4 对照实证: 小数据的训练量红利必须由增强解锁。
