...
- **命令**:
  ...
  **prod 缺省已自动使用冷学习率(lr 2e-5 / LoRA+ 1),无需显式传参;
  显式覆盖成更热组合会触发启动警告**(2026-07-21
...
### S2b2. 增强包 v2(已实现未启用;进配方前必过 from-scratch 消融)

- **开关**:`--augment-v2`(叠加在 `--augment` 之上,默认关)。三样全片
  一致:crop-zoom(裁 70~100%,放大近景身份细节,裁剪在 preprocess 拉伸
  384² 之前)/ 对比度 ±25%(围绕各帧均值,与 v1 亮度正交)/ 轻遮挡
  (p=0.3,全片同位置灰色矩形,模拟镜头污损)。
- **红线同 v1**:时序翻转(k↔l 标签互换)/mixup/TTA 不提供实现路径。
- **验证纪律(为什么现在不开)**:增强属慢显性手段——增量续训验证必
  假阴性,必须在**修正后干净标签的 100k 上 from-scratch 多 epoch 消融**
  (v2 开/关各一发,同 val 卷同 seed)过门才可进 1M 配方。1M 首跑维持
  v1 老三样。
