#!/bin/bash
# [EXP-FR2] 满分辨率老师蒸馏(B 机,双老师合议 soupw1+soupw3)——损失侧到顶(cb_pair 80.29)后的唯一正交轴。
...
T=outputs/soup_size
O=outputs/cb_fullres2_r64
mkdir -p $O
ts() { date '+%m-%d %H:%M'; }

for f in $T/student_r64.npz $T/soupw1_u512.npz $T/soupw3_u512.npz outputs/confuse_sw/pair_normal.json; do
  [ -s "$f" ] || { echo "[fr2 $(ts)] 缺 $f"; exit 1; }
done

echo "[fr2 $(ts)] 等 vfio 真空 …"
while :; do
  hold=0
...
echo "[fr2 $(ts)] 起训(满分辨率双老师 soupw1+soupw3, KD_TEACHER_FULLRES=1, dyn K=32 学生, uniform r64, pair_normal 加权, FOCAL_GAMMA=1.5, 500步 eval50 早停4)"
ATT=0
until RESUME=""; [ -f $O/ckpt_latest.npz ] && RESUME="--resume"; \
  ...
  --teacher-npz2 $T/soupw3_u512.npz \
...
[ -f $O/train_params_best.npz ] || { echo "[fr2] 无产物"; exit 1; }
...
calib() {
  [ -s "$1" ] || { echo "[fr2-calib $(ts)] 缺 $1"; return 1; }
  echo "[fr2-calib $(ts)] class_diag $3 ($(wc -l < $1) 行)"
...
echo "[fr2 $(ts)] ==== 满分辨率双老师合议蒸馏 公平校准汇总 ===="
