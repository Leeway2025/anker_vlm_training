#!/bin/bash
# [EXP-B3] focal gamma 1.5->2.0 (cb_pair 加强版, 冲最后0.13) 混淆对定向加权 + focal;直击 m 黑洞但不压 m 本身(修正 cb_aggr 的过压伤召回)
...
echo "[cb_pair $(ts)] 起训(dyn K=32, uniform r64, 双老师, pair_normal 定向加权, FOCAL_GAMMA=2.0, 500步 eval50 早停4)"
ATT=0
until RESUME=""; [ -f $O/ckpt_latest.npz ] && RESUME="--resume"; \
  SELECT_TOKENS_K=32 MAX_SOFT_TOKENS=64 TOKEN_COMPRESS_MODE=dyn FOCAL_GAMMA=2.0 \
