#!/bin/bash
# [EXP-B2] 混淆对定向加权 + focal;直击 m 黑洞但不压 m 本身(修正 cb_aggr 的过压伤召回)
...
until RESUME=""; [ -f $O/ckpt_latest.npz ] && RESUME="--resume"; \
  SELECT_TOKENS_K=32 MAX_SOFT_TOKENS=64 TOKEN_COMPRESS_MODE=dyn FOCAL_GAMMA=1.5 \
