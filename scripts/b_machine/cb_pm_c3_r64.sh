#!/bin/bash
# [EXP-pm-c3] pair-margin(coef=0.3,margin=2.0)叠加在 cb_pair(加权+focal1.5)之上;在 SubKS 位对同父兄弟施 hinge,补 focal 压不动的硬兄弟判别缺口
...
until RESUME=""; [ -f $O/ckpt_latest.npz ] && RESUME="--resume"; \
  SELECT_TOKENS_K=32 MAX_SOFT_TOKENS=64 TOKEN_COMPRESS_MODE=dyn FOCAL_GAMMA=1.5 PAIR_MARGIN_COEF=0.3 PAIR_MARGIN=2.0 \
