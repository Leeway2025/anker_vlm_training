#!/bin/bash
# 增强消融 · B机=增强臂(v1+v3 域定向,from-scratch)。用户 0811 07:46:增强=数据增强。
#   遵代码红线:慢显性增强"增量续训验证必假阴性"→必须 from-scratch 消融。
#   复刻 seed2 基座配方(投影暖启 proj_a + prod秩 + augment + accum32 + 1500步 +
#   eval100 + 早停3 + 固定val),与 A机 enh_aug_base 配对,唯一差异=本臂多 --augment-v3(灰度IR/低照度噪声/鱼眼warp/连续窗)。
#   只训练+推理+裸评测;校准在 A 机集中对两臂统一做(拉 B 的 eval_preds)。
cd /workspace && unset WDS_DIR
O=outputs/enh_aug_v3
mkdir -p $O
ATT=0
...
[ -f $O/train_params_best.npz ] || { echo "[enh_aug_v3] 无产物"; exit 1; }
