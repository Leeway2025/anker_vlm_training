#!/bin/bash
# Parallel eval on B of A's capmask_r64_dyn step100 best (CAP_WEIGHT=0, dyn K=32, r64).
# Mirrors the eval/quant/calib tail of scripts/capmask_r64_dyn.sh. Output: outputs/capmask_A_eval.
set -e
cd /workspace && unset WDS_DIR
O=outputs/capmask_A_eval
ts() { date '+%m-%d %H:%M'; }
echo "[Beval $(ts)] start on $(hostname); ckpt:"
ls -la $O/train_params_best.npz
[ -s $O/train_params_best.npz ] || { echo "[Beval] no ckpt"; exit 1; }

echo "[Beval $(ts)] fp32 dyn infer (K=32)"
...
echo "[Beval $(ts)] int8 quantize + dyn infer"
...
echo "[Beval $(ts)] ==== A step100 capmask (CAP_WEIGHT=0) on B ===="
echo "  fp32: $(cat $O/fair_calib_fp32.txt 2>/dev/null | tr '\n' ' ')"
echo "  int8: $(cat $O/fair_calib_int8.txt 2>/dev/null | tr '\n' ' ')"
echo "  line 87.91/80.42 ; champ ens_r64_dyn int8=80.16/fp32=80.12"
echo "[Beval done $(ts)]"
