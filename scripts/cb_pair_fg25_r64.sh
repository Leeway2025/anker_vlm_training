#!/bin/bash
# [EXP-A-fg25] focal gamma=2.5 (cb_pair 加强; 与B机 gamma2.0 夹 focal 最优点)
...
echo "[cb_pair $(ts)] 起训(dyn K=32, uniform r64, 双老师, pair_normal 定向加权,, 500步 eval50 早停4)"
ATT=0
until RESUME=""; [ -f $O/ckpt_latest.npz ] && RESUME="--resume"; \
  SELECT_TOKENS_K=32 MAX_SOFT_TOKENS=64 TOKEN_COMPRESS_MODE=dyn FOCAL_GAMMA=2.5 \
  ...
  --steps 500 --eval-every 50 --early-stop-patience 4 --ckpt-every 600 \
