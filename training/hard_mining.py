...
⚠️ 朴素"错例 ×3"的三个下滑风险与对应防护(2026-07-08 加固):
  ...
  ② "模型错了"里可能混着"GT 错了" → --whitelist(资产 D)只放大
    Gemini 认同 GT 的错例。⚠️ 适用性取决于 GT 来源(2026-07-08 确认):
    - 客户正式数据 GT = Gemini 标注 + **人工修正** → GT 可信,错例
      基本都真,**不要传 --whitelist**(Gemini 不认同的恰是人工看出、
      Gemini 看不出的盲区难例,过滤=错杀最有价值样本)
    - 代理集伪 GT(纯 Gemini 双温度)→ 无人工背书,**建议传**
